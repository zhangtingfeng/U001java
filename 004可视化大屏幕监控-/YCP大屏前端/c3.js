...
(function (global, factory) {
    ...
    c3_chart_internal_fn.initSubchart = function () {
        ...
        // ★fix start for add subchart class.
        // Define g for chart area
        // context.append('g').attr("clip-path", $$.clipPathForSubchart).attr('class', CLASS.chart);
        context.append('g').attr("clip-path", $$.clipPathForSubchart).attr('class', CLASS.chart + ' subchart-' + CLASS.chart);

        // Define g for bar chart area
        // context.select('.' + CLASS.chart).append("g").attr("class", CLASS.chartBars);
        context.select('.' + CLASS.chart).append("g").attr("class", CLASS.chartBars + ' subchart-' + CLASS.chartBars);

        // Define g for line chart area
        // context.select('.' + CLASS.chart).append("g").attr("class", CLASS.chartLines);
        context.select('.' + CLASS.chart).append("g").attr("class", CLASS.chartLines + ' subchart-' + CLASS.chartLines);

        // Add extent rect for Brush
        // context.append("g").attr("clip-path", $$.clipPath).attr("class", CLASS.brush).call($$.brush);
        context.append("g").attr("clip-path", $$.clipPath).attr("class", CLASS.brush + ' subchart-' + CLASS.brush).call($$.brush);
        ...
        $$.axes.subx = context.append("g").attr("class", CLASS.axisX + ' subchart-' + CLASS.axisX).attr("transform", $$.getTranslate('subx')).attr("clip-path", config.axis_rotated ? "" : $$.clipPathForXAxis).style("visibility", config.subchart_axis_x_show ? visibility : 'hidden');
        // ★fix end for add subchart class.
    };
    ...
    c3_chart_internal_fn.getYForText = function (points, d, textElement) {
        ...
        if ($$.config.axis_rotated) {
            ...
            yPos = points[2][1];
            // ★fix start for 0 label height change.
            // if (d.value < 0 || d.value === 0 && !$$.hasPositiveValue) {
            if (d.value < 0) {
            // ★fix end for 0 label height change.
                yPos += box.height;

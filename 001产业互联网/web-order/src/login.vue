		...
		<div class="login-box-body">
			...
			<form action method="post" @submit.prevent="login">
				...
				<!-- <div class="form-group">
					...
					<div class="input-group captcha-imgs">
						<input type="text" class="form-control" v-model="form.captcha" @focus.once="loadCaptcha">
						<span class="input-group-addon">
							<imgs :src="captchaImg" @click="loadCaptcha" v-if="captchaImg"/>
						</span>
					</div>
        </div>-->
				<div class="row">
...
export default {
	...
	methods: {
		// loadCaptcha() {
		//   this.captchaImg = "api/user/captcha.png?" + new Date().getTime();
		// },
		async login() {
			if (this.loging) {
				return
...
/*
.captcha-imgs .input-group-addon {
  padding: 0;
}
.captcha-imgs .input-group-addon imgs {
  height: 32px;
  cursor: pointer;
}
*/

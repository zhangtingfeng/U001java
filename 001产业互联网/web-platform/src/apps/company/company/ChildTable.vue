        ...
        <tr v-bind:key="idx" v-if="d.$op != -1">
          ...
          <template v-for="(f) of meta.fields">
            <td v-bind:key="f.name" v-if="f.form != 0">
              <input v-if="!f.editor" :name="f.name" class="form-control" v-model="d[f.name]" v-validate="f.validate ? f.validate : ''" />
              <component v-else :is="getEditor(f.editor)" :args="f.editorArgs" :name="f.name" v-model="d[f.name]" :validate="f.validate ? f.validate : ''" />
            </td>
          </template>
        </tr>
      </template>

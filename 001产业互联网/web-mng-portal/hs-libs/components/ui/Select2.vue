...
export default {
  data() {
    return { data: this.data2(this.value) };
  },
  props: {
    options: { default: [] },
    multi: { default: false },
  ...
  watch: {
    data(val, oldval) {
      this.$emit(
        "input",
        this.multi && Array.isArray(val) ? val.join(",") : val
      );
    },
    value() {
  ...
  methods: {
    data2(v) {
      if (this.multi) return Array.isArray(v) ? v : v ? v.split(",") : [];
      else return v;
    },
    uninit() {
    ...
    updateOptions() {
      let hint = this.hint || this.$t("select");
      let options = (this.multi
        ? []
        : [{ id: "", text: `-- ${hint} --` }]
      ).concat(this.options);
      ...
      this.updateValue();
      this.$nextTick(() => {
        this.$select.on("change.select2", (e) => {
          this.data = this.$select.val();
